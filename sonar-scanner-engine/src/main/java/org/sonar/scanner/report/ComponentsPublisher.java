...
public class ComponentsPublisher implements ReportPublisherStep {
  ...
  private boolean shouldSkipComponent(DefaultInputComponent component, Collection<InputComponent> children) {
    if (component instanceof InputModule && children.isEmpty()
      && (branchConfiguration.isShortLivingBranch() || branchConfiguration.isPullRequest())) {
      ...
      return !inputFile.isPublished() || ((branchConfiguration.isShortLivingBranch() || branchConfiguration.isPullRequest()) && inputFile.status() == Status.SAME);
    }
    return false;
  }
